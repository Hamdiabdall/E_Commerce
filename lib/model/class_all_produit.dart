...
class AllProducts {
  final List<ClassProduit> _products = [
    ClassProduit(
      id: "1",
  ...
  // Get all products
  List<ClassProduit> get products {
    return [..._products];
  }

  // Get favorite products
  List<ClassProduit> get favoriteProducts {
    return _products.where((produit) => produit.isFavorite).toList();
  }

  // Find product by ID
  ClassProduit findById(String id) {
    return _products.firstWhere((produit) => produit.id == id);
  }

  // Get products by category
  List<ClassProduit> getProductsByCategory(String categoryName) {
    return _products.where((produit) => produit.produitCategoryName == categoryName).toList();
  }

  // Add a new product
  void addProduct(ClassProduit produit) {
    _products.add(produit);
  }

  // Remove a product
  void removeProduct(String id) {
    _products.removeWhere((produit) => produit.id == id);
  }
